...
test("server-renders the finished Binance futures dashboard", async () => {
  ...
  assert.match(workspaceCss, /\.timeframeOption span\s*\{[^}]*width:\s*100%[^}]*min-width:\s*0[^}]*height:\s*32px/s);
  ...
  assert.match(workspaceCss, /\.timeframeControls select\s*\{[^}]*height:\s*32px/s);
  assert.match(
    workspaceCss,
